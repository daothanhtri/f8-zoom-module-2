...
import { getPopularTracks } from "../api/tracks.js";
import { getTrendingArtists } from "../api/artists.js";
import playerModule from "../components/player.js"; // <-- Import playerModule

const homeSections = getElement("#homeSections");
const hitsGrid = getElement("#hitsGrid");
const artistsGrid = getElement("#artistsGrid");

let navigateToCallback = null; // Callback for app-level navigation
...
const createCard = (item, type) => {
  ...
  let subtitle;
  let playButtonClass; // Class for the play button on the card
  let playButtonElement; // The actual play button element
  ...
  if (type === "track") {
    ...
    // type === "artist"
    imageUrl =
      item.avatar_url || "https://via.placeholder.com/160?text=No+Avatar";
    subtitle = "Artist";
  ...
  // Get the play button element within the newly created card
  playButtonElement = card.querySelector(`.${playButtonClass}`);

  // Handle click on the entire card (for navigation to detail page)
  card.addEventListener("click", (e) => {
    // Only navigate if click is not on the play button
    if (
  ...
  if (type === "track" && playButtonElement) {
    // Only tracks are playable in this context
...
export const renderHomePage = async () => {
  ...
  let loadedTracks = [];
  try {
    const tracksResponse = await getPopularTracks();
